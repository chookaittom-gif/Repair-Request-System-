...
const CONFIG = {
  ...
  // 🔽 โลโก้ระบบและรายงาน PDF
  LOGO_FOLDER_ID: PropertiesService.getScriptProperties().getProperty('LOGO_FOLDER_ID') || '1VXuTOnSEv23_eVFlYihNEqnZzDUuKwzd',
  LOGO_FILE_NAME: PropertiesService.getScriptProperties().getProperty('LOGO_FILE_NAME') || 'SDULP-FIX.png',
...
function doGet(e) {
  try {
    ...
    if (action === 'serveLogo') {
      const logoId = resolveLogoFileId_();
      if (logoId) {
        return serveImage(logoId);
      }
    }
    ...
    template.logoUrl = getLogoPublicUrl();
...
function submitRepairForm(formData, imageFiles) {
  try {
    if (!formData || !formData.requesterName) {
    ...
    try { CacheService.getScriptCache().remove('dashboard:data:v4'); } catch(e) {}
...
function buildNewRepairNotificationData_(formData, sequenceNumber) {
  ...
  return {
    ...
    '{{รายการแจ้งซ่อม}}': String(formData.description || formData.repairItem || '-').trim(),
...
function generateSequenceNumber() {
  ...
  try {
    ...
    values.forEach(row => {
      ...
      if (existingNumber) {
        const str = existingNumber.toString().trim();
        if (str.startsWith(datePrefix)) {
...
function saveToSheet(formData) {
  ...
  try {
    ...
    set('สถานะล่าสุด', STATUS.PENDING);
  ...
  try {
    ...
    const data = values.slice(1).map((row, i) => {
      ...
      return {
        ...
        status: normalizeStatusName_(String(row[colMap.status] || '').trim() || STATUS.PENDING),
        statusLatest: normalizeStatusName_(String(row[colMap.status] || '').trim() || STATUS.PENDING),
        assignedTechnician: row[colMap.tech] || '-',
        imageUrls: fileIds.map(id => convertFileIdToUrl(id)),
        phoneNumber: row[colMap.phone],
        repairType: (() => {
          const t = String(row[colMap.type] || '').trim();
          if (t === 'ไฟฟ้า') return 'ไฟฟ้าและวิทยุ';
          if (t === 'แอร์') return 'เครื่องปรับอากาศ';
          if (t === 'ทั่วไป' || !t) return 'อื่นๆ';
          return t;
        })(),
...
/**
 * ค้นหาข้อมูลแจ้งซ่อมของตนเองด้วยเลขที่แจ้งซ่อม (Exact Match) หรือเบอร์โทรศัพท์ (Normalized Match)
 * มีการเซนเซอร์เบอร์โทรศัพท์เพื่อความปลอดภัยของข้อมูล (Privacy)
 * @param {string} query คำค้นหา
 * @return {object} ผลลัพธ์การค้นหา
 */
function searchMyRepairTickets(query) {
  try {
    ...
    const sheet = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID).getSheetByName(CONFIG.SHEET_NAME);
    if (!sheet) {
      throw new Error('ไม่พบชีตข้อมูล');
    }
    ...
    const cSeq = getColIndex(['เลขที่']);
    const cDate = getColIndex(['วันที่']);
    const cOwner = getColIndex(['ชื่อผู้แจ้งซ่อม', 'ผู้แจ้ง']);
    const cPhone = getColIndex(['เบอร์โทร', 'เบอร์โทรศัพท์', 'เบอร์']);
    const cItem = getColIndex(['รายการแจ้งซ่อม', 'รายการ']);
    const cLoc = getColIndex(['สถานที่']);
    const cStat = getColIndex(['สถานะล่าสุด', 'สถานะ']);
    const cPrio = getColIndex(['ความเร่งด่วน']);
    const cDesc = getColIndex(['อาการ']);

    if (cSeq === -1 || cDate === -1 || cPhone === -1 || cStat === -1 || cItem === -1 || cOwner === -1 || cLoc === -1 || cPrio === -1 || cDesc === -1) {
      throw new Error('โครงสร้างคอลัมน์ของชีตไม่ถูกต้อง');
    }

    const qUpper = cleanQuery.toUpperCase();
    const qPhone = cleanQuery.replace(/[^\d]/g, '');
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      const dispRow = displayValues[i + 1];
      const seqVal = String(row[cSeq] || '').trim().toUpperCase();
      const phoneVal = String(dispRow[cPhone] || row[cPhone] || '').trim();
      const phoneDigits = phoneVal.replace(/[^\d]/g, '');

      let isMatch = false;

      // ค้นหาแบบ Exact Match ด้วยเลขที่ (ตัวใหญ่และไม่มีช่องว่าง)
      if (seqVal === qUpper) {
        isMatch = true;
      }
      // หรือ ค้นหาด้วยเบอร์โทรศัพท์ (เปรียบเทียบเฉพาะหลักตัวเลขและรองรับเลขศูนย์หน้าหาย)
      else if (qPhone && phoneDigits) {
        if (qPhone === phoneDigits) {
          isMatch = true;
        } else if (qPhone.startsWith('0') && qPhone.length === 10 && qPhone.slice(1) === phoneDigits) {
          isMatch = true;
        } else if (phoneDigits.startsWith('0') && phoneDigits.length === 10 && phoneDigits.slice(1) === qPhone) {
          isMatch = true;
        }
      }

      if (isMatch) {
        // ทำ Mask เบอร์โทรศัพท์ เช่น 081-xxx-5678
        let phoneMasked = '';
        if (phoneDigits.length >= 9) {
          const prefix = phoneDigits.startsWith('0') ? phoneDigits.slice(0, 3) : '0' + phoneDigits.slice(0, 2);
          const suffix = phoneDigits.slice(-4);
          phoneMasked = prefix + '-xxx-' + suffix;
        } else if (phoneVal.length > 4) {
          phoneMasked = phoneVal.slice(0, 2) + '-xxx-' + phoneVal.slice(-2);
        } else {
          phoneMasked = '***';
        }
        ...
        results.push({
          ...
          date: formatThaiDateTime(row[cDate]),
          requesterName: String(row[cOwner] || '').trim(),
          phoneMasked: phoneMasked,
          repairItem: String(row[cItem] || '').trim(),
          location: String(row[cLoc] || '').trim(),
          status: String(row[cStat] || '').trim(),
          priority: String(row[cPrio] || '').trim(),
          description: String(row[cDesc] || '').trim()
        });
      }
    }

    // เรียงให้รายการล่าสุดขึ้นก่อน
    return { success: true, data: results.reverse() };
  } catch (error) {
    Logger.log('Error in searchMyRepairTickets: ' + error.stack);
    return { success: false, error: error.message };
  }
}
...
function listReports(options = {}) {
  try {
    ...
    const range = sheet.getDataRange();
    const values = range.getValues();
    const dispValues = range.getDisplayValues();
    ...
    const cPhone = getColIndex(['เบอร์โทร', 'เบอร์โทรศัพท์', 'เบอร์']);
    const cLoc = getColIndex(['สถานที่']);
    const cType = getColIndex(['ประเภทงานซ่อม', 'ประเภท']);
    const cStat = getColIndex(['สถานะล่าสุด', 'สถานะ']);
    const cPrio = getColIndex(['ความเร่งด่วน']);

    const dataRows = values.slice(1).map((row, i) => ({
      raw: row,
      disp: dispValues[i + 1]
    }));

    const kw = (q || '').toString().trim().toLowerCase();
    const normKw = kw.replace(/[^\d]/g, '');
    const seqKw = kw.toUpperCase().replace(/\s+/g, '');
    const statusKw = status ? normalizeStatusName_(cleanFilterText(status)) : '';
    ...
    const filtered = dataRows.filter(item => {
      ...
      if (statusKw) {
        const statusVal = normalizeStatusName_(cleanFilterText(cStat > -1 ? (r[cStat] || d[cStat]) : ''));
        if (statusVal !== statusKw) return false;
      }
      ...
      let match = false;
      if (cSeq > -1) {
        const seqVal = String(r[cSeq] || '').trim().toUpperCase().replace(/\s+/g, '');
        if (seqVal.includes(seqKw)) match = true;
      }
      if (!match && cPhone > -1) {
        const phoneVal = String(d[cPhone] || r[cPhone] || '').trim();
        const normPhone = phoneVal.replace(/[^\d]/g, '');
        if (normKw && normPhone.includes(normKw)) match = true;
      }
      if (!match && cOwner > -1) {
        const ownerVal = String(r[cOwner] || '').toLowerCase();
        if (ownerVal.includes(kw)) match = true;
      }
    ...
    filtered.sort((a, b) => {
      const dateA = (cDate > -1 && a.raw[cDate] instanceof Date) ? a.raw[cDate] : new Date(0);
      const dateB = (cDate > -1 && b.raw[cDate] instanceof Date) ? b.raw[cDate] : new Date(0);
    ...
    const mapped = paginatedRows.map(item => {
      const r = item.raw;
      const d = item.disp;
      const phoneVal = cPhone > -1 ? String(d[cPhone] || r[cPhone] || '').trim() : '';
      ...
      return {
        ...
        loc: cLoc > -1 ? r[cLoc] : '',
        type: (() => {
          const t = cType > -1 ? String(r[cType] || '').trim() : '';
          if (t === 'ไฟฟ้า') return 'ไฟฟ้าและวิทยุ';
          if (t === 'แอร์') return 'เครื่องปรับอากาศ';
          if (t === 'ทั่วไป' || !t) return 'อื่นๆ';
          return t;
    ...
    return { ok: true, rows: mapped, total, page, pageSize, filters: { status: statusKw, pageSize: pageSize } };
...
function getPendingWithdrawalRequestCountForDashboard(currentUser) {
  try {
    ...
    const ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
    const sheet = ss.getSheetByName('คำขอเบิกอะไหล่');
    ...
    const statusCol = headers.indexOf('สถานะคำขอ') + 1;
    if (!statusCol) {
      throw new Error('ไม่พบคอลัมน์ สถานะคำขอ');
    }

    const values = sheet.getRange(2, statusCol, sheet.getLastRow() - 1, 1).getValues();
    let count = 0;
    values.forEach(row => {
      if (String(row[0] || '').trim() === 'รออนุมัติ') count++;
    });

    return { success: true, count: count };
  } catch (error) {
    Logger.log('[DASHBOARD_WITHDRAWAL_BADGE] ' + (error && error.stack ? error.stack : error));
    return {
      success: false,
...
function getDashboardData(currentUser) {
  const cache = CacheService.getScriptCache();
  const cacheKey = 'dashboard:data:v4';
  ...
  try {
    ...
    if (cached) {
      ...
      const rawData = Array.isArray(publicDataResult.data) ? publicDataResult.data : [];
      
      // Normalize statuses of repair list, excluding withdrawal status categories from repair dashboard
      const data = rawData.map(item => {
        return {
          ...item,
          status: normalizeStatusName_(item.status)
        };
      }).filter(item => {
        const s = item.status;
        return s !== 'รออนุมัติ' && s !== 'อนุมัติ' && s !== 'ไม่อนุมัติ' && s !== 'จ่ายแล้ว' && s !== 'ยกเลิก';
      });
    ...
    const withdrawalRes = getPendingWithdrawalRequestCountForDashboard(currentUser);
    result.pendingWithdrawalCount = withdrawalRes.success ? withdrawalRes.count : 0;
...
function normalizeStatusName_(input) {
  ...
  if (!s || s === '-') return STATUS.PENDING;
  const partsStatuses = [
    'พัสดุอยู่ระหว่างรอเบิก',
    'รอเบิกพัสดุ',
    'รอพัสดุ',
    'รอรับพัสดุ',
    'รออะไหล่',
...
function updateRepairStatus(payload, newStatus, technician, notes, currentUser, base64Images) {
  ...
  try {
    ...
    let status = normalizeStatusName_(p.newStatus || p.status || p.state || '');
    const tech = String(p.technician || p.tech || p.assignee || '').trim();
    const noteContent = String(p.notes || p.note || '').trim();
    if (!noteContent) throw new Error('กรุณาระบุบันทึกการทำงาน');
    ...
    if (p.isAssignment) {
      status = normalizeStatusName_(sheet.getRange(rowIndex, colStatus).getValue());
    }
    ...
    if (!p.isAssignment) {
      ...
      const colAssignDate = map['วันที่มอบหมาย'];
      const colAssigner = map['ผู้มอบหมาย'];
      const colAssignHistory = map['ประวัติการมอบหมาย'];
      if (colAssignDate) sheet.getRange(rowIndex, colAssignDate).setValue(actionDate);
      if (colAssigner) sheet.getRange(rowIndex, colAssigner).setValue(getStatusLogUserName_(userObj));
      if (colAssignHistory) {
        const curHist = String(sheet.getRange(rowIndex, colAssignHistory).getValue() || '');
        const newHist = `[${Utilities.formatDate(actionDate, tz, 'dd/MM HH:mm')}] มอบหมายให้: ${tech} โดย: ${getStatusLogUserName_(userObj)}`;
        sheet.getRange(rowIndex, colAssignHistory).setValue(curHist ? `${curHist}\n${newHist}` : newHist);
      }
    }
...
function selfTestStatusLogReportMapping_() {
  const values = [
    ...
    [new Date(2026, 0, 1, 11, 0), 'SDUL-OTHER', STATUS.PENDING, 'รอซ่อม', 'Admin Test', '']
  ];
...
function selfTestStatusHistoryLayoutEstimate_() {
  ...
  const rows = [
    ...
    { data: ['01/01/2569', 'รอดำเนินการ', '-', 'Admin', ''], imgIds: [] }
  ];
...
function selfTest() {
  ...
  const runTest = (name, fn) => {
    try {
      const res = fn();
      if (res && res.success) {
        logs.push(`✅ ${name} PASS: ${res.message || 'สำเร็จ'}`);
      } else {
        logs.push(`❌ ${name} FAIL: ${res ? res.error : 'ล้มเหลว'}`);
      }
    } catch (e) {
      logs.push(`❌ ${name} FAIL: ${e.message}`);
    }
  };

  runTest('New Repair Mapping', selfTestNewRepairNotificationDataMapping_);
  runTest('Payload Auth', selfTestUpdateRepairStatusPayloadAuth_);
  runTest('Image Scope', selfTestStatusUpdateLatestImageScope_);
  runTest('Status Log Mapping', selfTestStatusLogReportMapping_);
  runTest('Status Header Mapping', selfTestUpdateRepairStatusHeaderMapping_);
  runTest('Report Status Header Mapping', selfTestReportStatusHeaderMapping_);
  runTest('Report Image Header Mapping', selfTestReportImageHeaderMapping_);
  runTest('Status History Layout Estimate', selfTestStatusHistoryLayoutEstimate_);
...
function generateChartData(data) {
  const stats = {
    statusCount: { pending: 0, processing: 0, external: 0, parts: 0, completed: 0 },
    repairTypeCount: { electrical: 0, plumbing: 0, aircon: 0, computer: 0, appliance: 0, officeSupply: 0, houseKitchen: 0, other: 0 },
    technicianStats: {}
  };
  ...
  data.forEach(item => {
    ...
    const typeMap = {
      'ทั่วไป': 'other',
      'ไฟฟ้า': 'electrical',
      'ไฟฟ้าและวิทยุ': 'electrical',
      'ประปา': 'plumbing',
      'แอร์': 'aircon',
      'เครื่องปรับอากาศ': 'aircon',
      'คอมพิวเตอร์': 'computer',
      'เครื่องใช้ไฟฟ้า': 'appliance',
      'วัสดุสำนักงาน': 'officeSupply',
      'วัสดุงานบ้าน งานครัว': 'houseKitchen'
    };
...
/**
 * ค้นหาและคืนค่า File ID ของโลโก้ (SDULP-FIX.png) จาก Drive Folder โดยอัตโนมัติ พร้อม Cache
 */
function resolveLogoFileId_() {
  if (CONFIG && CONFIG.REPORT_LOGO_FILE_ID && String(CONFIG.REPORT_LOGO_FILE_ID).trim()) {
    return String(CONFIG.REPORT_LOGO_FILE_ID).trim();
  }

  const cache = CacheService.getScriptCache();
  const cachedId = cache.get('SDULP_LOGO_FILE_ID');
  if (cachedId) {
    return cachedId;
  }

  try {
    const folderId = (CONFIG && CONFIG.LOGO_FOLDER_ID) ? String(CONFIG.LOGO_FOLDER_ID).trim() : '1VXuTOnSEv23_eVFlYihNEqnZzDUuKwzd';
    const fileName = (CONFIG && CONFIG.LOGO_FILE_NAME) ? String(CONFIG.LOGO_FILE_NAME).trim() : 'SDULP-FIX.png';
    if (!folderId) return '';
    ...
    if (files.hasNext()) {
      ...
      try { cache.put('SDULP_LOGO_FILE_ID', fileId, 21600); } catch(e) {}
      return fileId;
    }
  } catch (e) {
    Logger.log('[WARN] resolveLogoFileId_ error: ' + e.message);
  }
  return '';
}

/**
 * ส่งคืน URL สาธารณะของโลโก้สำหรับใช้ในหน้าเว็บ
 */
function getLogoPublicUrl() {
  const fileId = resolveLogoFileId_();
  if (fileId) {
    return 'https://lh3.googleusercontent.com/d/' + fileId;
  }
  return 'https://img2.pic.in.th/pic/SDULP-FIX52933b8334b3dd59.png';
}
...
function sendNewRepairNotification(formData, docUrl, imageUrls = []) {
  try {
    ...
    const lines = [
      `🆕 แจ้งซ่อมใหม่: ${sanitizeHtml(seq)}`,
      `━━━━━━━━━━━━━━`,
      `👤 ผู้แจ้ง: ${sanitizeHtml(requesterName)}`,
      `📞 เบอร์โทร: ${sanitizeHtml(phoneNumber)}`,
      `📍 สถานที่: ${sanitizeHtml(location)}`,
      `🔧 ประเภทงาน: ${sanitizeHtml(repairType)}`,
      `📝 อาการ/รายละเอียดปัญหา: ${sanitizeHtml(description)}`,
      `${priorityIcon} ความเร่งด่วน: ${sanitizeHtml(priority || 'ปานกลาง')}`
    ];
...
function sendStatusUpdateNotification(originalRowData, headers, newStatus, assignedTechnician, notes, userInfo, latestStatusImageUrls) {
  try {
    ...
    const description = String(getVal('อาการ') || '-').trim();
    ...
    const now = new Date();
    const tz = Session.getScriptTimeZone() || 'Asia/Bangkok';
    const dayMonth = Utilities.formatDate(now, tz, 'dd/MM/');
    const yearBE = now.getFullYear() + 543;
    const time = Utilities.formatDate(now, tz, 'HH:mm');
    const timeStr = dayMonth + yearBE + ' ' + time + ' น.';

    const normalizedStatus = normalizeStatusName_(currentStatus);
    let statusEmoji = '🔄';
    let statusLabel = currentStatus;
    if (normalizedStatus === STATUS.PENDING) {
      statusEmoji = '🆕';
      statusLabel = 'งานใหม่';
    } else if (normalizedStatus === STATUS.PROCESSING) {
      statusEmoji = '🛠';
      statusLabel = 'กำลังดำเนินการ';
    } else if (normalizedStatus === STATUS.EXTERNAL) {
      statusEmoji = '🚚';
      statusLabel = 'ดำเนินการภายนอก';
    } else if (normalizedStatus === STATUS.PARTS) {
      statusEmoji = '📦';
    } else if (normalizedStatus === STATUS.COMPLETED) {
      statusEmoji = '✅';
    ...
    const updateLines = [];
    const jobTitle = description && description !== '-' ? description : (repairItem && repairItem !== '-' ? repairItem : '');
    if (jobTitle && jobTitle !== '-') {
      updateLines.push(`📝 ${sanitizeHtml(jobTitle)}`);
    }
    ...
    if (noteFinal) {
      lines.push(``, `📝 บันทึกการทำงาน`, sanitizeHtml(noteFinal));
    }
  ...
  try {
    ...
    if (safeImageUrls.length > 0) {
      if (safeImageUrls.length > 1) {
        ...
        try {
          const urls = safeImageUrls.map(item => {
            const s = String(item || '').trim();
            if (!s) return null;
            if (s.startsWith('http://') || s.startsWith('https://')) {
              return s;
            }
            const fileId = extractFileIdFromUrl(s);
            if (fileId) {
              return typeof convertFileIdToUrl === 'function' ? convertFileIdToUrl(fileId) : `https://lh5.googleusercontent.com/d/${fileId}`;
            }
            return `https://lh5.googleusercontent.com/d/${s}`;
          }).filter(Boolean);

          if (urls.length > 0) {
            const mediaArray = urls.map(url => ({
              type: 'photo',
              media: url
            }));

            const response = UrlFetchApp.fetch(`${telegramUrl}/sendMediaGroup`, {
              method: 'post',
              contentType: 'application/json',
            ...
            Logger.log('📸 ส่งกลุ่มรูปภาพ (Media Group) สำเร็จ: ' + response.getContentText());
          }
        } catch (mediaGroupErr) {
          Logger.log('⚠️ ส่งกลุ่มรูปภาพ (sendMediaGroup) ไม่สำเร็จ: ' + mediaGroupErr.toString());
        }
      } else {
        // Single photo
...
function createMonthlyReportDocument(year, month, selectedTechnician) {
  ...
  try {
    ...
    const extractImageIds = function(val) {
      ...
      const rawItemVal = colMap.item > -1 && rowRaw[colMap.item] ? String(rowRaw[colMap.item]).trim() : '';
      const descVal = colMap.desc > -1 && rowRaw[colMap.desc] ? String(rowRaw[colMap.desc]).trim() : '';
      const itemVal = descVal || rawItemVal || '-';
...
function helpTakeReportPlaceholderTargetV2_(body, placeholder) {
  const found = body.findText(helpEscapeReportPlaceholderPattern_(placeholder));
  ...
  let paragraph = text.getParent();
  while (paragraph && paragraph.getType && paragraph.getType() !== DocumentApp.ElementType.PARAGRAPH) {
    paragraph = paragraph.getParent();
  }
  if (!paragraph || !paragraph.getParent) return null;
  ...
  try {
    ...
    Logger.log('[REPORT] placeholder target ไม่ใช่ child ของ container: ' + placeholder + ' | ' + e.message);
    return null;
  }
  ...
  try {
    canRemoveBlankParagraph =
      paragraph.asParagraph().getText().trim() === '' &&
  ...
  if (canRemoveBlankParagraph) {
    try {
      ...
      Logger.log('[REPORT] ลบ paragraph placeholder target ไม่สำเร็จ: ' + placeholder + ' | ' + e.message);
      return { container: container, index: index + 1 };
    }
  }
...
function helpCompactReportParagraphV2_(p, spacingBefore, spacingAfter) {
  if (!p) return p;
  try {
    p.setSpacingBefore(typeof spacingBefore === 'number' ? spacingBefore : 0)
      .setSpacingAfter(typeof spacingAfter === 'number' ? spacingAfter : 0)
      .setLineSpacing(1);
  } catch (e) {}
  return p;
}

function helpCompactReportCellV2_(cell, width) {
  if (!cell) return cell;
  try {
    cell.setPaddingTop(2)
      ...
      .setPaddingLeft(4)
      .setPaddingRight(4);
    if (typeof width === 'number' && width > 0) cell.setWidth(width);
  } catch (e) {}
  return cell;
}

function helpInsertReportPageBreakAtCursorV2_(container, cursor) {
  if (!container || typeof cursor !== 'number' || cursor < 0) return false;
  try {
    helpCompactReportParagraphV2_(helpInsertReportParagraphV2_(container, cursor, ''))
      .appendPageBreak();
    return true;
  } catch (e) {
    Logger.log('[REPORT] insert page break at cursor failed: ' + e.message);
    return false;
  }
}
...
function generatePdfFileFromRows(pdfRows, tableData, colWidths, targetMonth, displayYearBE, foundCount, completedCount, processingCount, externalCount, options) {
  ...
  doc.setMarginLeft(18).setMarginRight(18).setMarginTop(18).setMarginBottom(30);
  try {
  ...
  const tableTarget = helpTakeReportPlaceholderTargetV2_(body, '{{ตารางสรุป}}');
  let tableContainer = tableTarget && tableTarget.container ? tableTarget.container : body;
  let tableInsertIndex = tableTarget ? tableTarget.index : -1;
  if (tableInsertIndex < 0) {
    tableInsertIndex = helpTakeReportTableTitleIndexV2_(body, 'ตารางรายการแจ้งซ่อม', '{{ตารางสรุป}}');
    tableContainer = body;
  }
  ...
  let currentTable = helpInsertReportTableV2_(tableContainer, tableCursor);
  try {
  ...
  renderRows.forEach(function(row) {
    ...
    if (currentHeight + estimatedRowHeight > PAGE_LIMIT) {
      if (tableCursor >= 0) {
        const tableParent = currentTable && currentTable.getParent ? currentTable.getParent() : tableContainer;
        const pageBreakParagraph = helpInsertReportParagraphV2_(tableParent, tableCursor, '');
        pageBreakParagraph
          .setSpacingBefore(0)
          .setSpacingAfter(0)
          .setLineSpacing(1)
          .appendPageBreak();
        tableCursor++;
        currentTable = helpInsertReportTableV2_(tableParent, tableCursor);
        tableCursor++;
      } else {
      ...
      if (currentTable && currentTable.getParent()) {
        try {
          const tableParent = currentTable.getParent();
          const tableIndex = tableParent.getChildIndex(currentTable);
          if (tableIndex > 0) {
            const prev = tableParent.getChild(tableIndex - 1);
  ...
  const historyTarget = helpTakeReportPlaceholderTargetV2_(body, '{{ประวัติสถานะ}}');
  const historyContainer = historyTarget && historyTarget.container ? historyTarget.container : body;
  const historyInsertIndex = historyTarget ? historyTarget.index : -1;
  appendStatusHistoryToReport_(historyContainer, rows, font, historyInsertIndex, {
    renderImages: !hasImageSectionPlaceholder
  });
  appendReportImageSectionV2_(body, rows, font, {
    allowAppendFallback: false
  });
  ...
  cleanupTrailingBlankContent_(body);

  // Task 1: Add page numbers to Footer
  // หมายเหตุ: ใน Google Apps Script DocumentApp ไม่มีฟังก์ชัน native เช่น appendPageNumber() หรือ appendPageCount()
  // หากพยายามเรียกจะเกิด TypeError ทำให้โค้ดพังและแครชเงียบใน catch block เสมอ
  // แนวทางที่ถูกต้องและมินิมอลที่สุดคือ:
  // 1. ถ้าใช้ Template (usedReportTemplate = true): จะไม่เขียนทับหรือเคลียร์ footer เพื่อคงหน้าตาและเลขหน้าที่ผู้ใช้ตั้งค่าไว้จาก Google Doc UI (Insert > Page numbers)
  // 2. ถ้าไม่ได้ใช้ Template: จะหลีกเลี่ยงการใช้คำสั่งที่ไม่มีจริงเพื่อไม่ให้แครช และแจ้งข้อมูลเบื้องต้น
  if (!usedReportTemplate) {
    try {
      const footer = doc.getFooter() || doc.addFooter();
      let pFooter = footer.getChild(0) ? footer.getChild(0).asParagraph() : null;
      if (!pFooter) {
        pFooter = footer.appendParagraph('');
      } else {
        pFooter.setText('');
      }
      pFooter.setAlignment(DocumentApp.HorizontalAlignment.RIGHT)
             .setFontFamily(font)
             .setFontSize(9)
             .setForegroundColor('#000000')
             .setText('');
    } catch (e) {
      Logger.log('[WARN] Failed to insert fallback footer: ' + e.message);
    }
  }

  // Task 2: Force all text in document body to Black
  try {
    body.setForegroundColor('#000000');
  } catch (e) {
    Logger.log('[WARN] Failed to set body text color to black: ' + e.message);
  }
...
function appendStatusHistoryToReport_(body, rows, font, insertIndex, options) {
  ...
  const allowAppendFallback = !!(options && options.allowAppendFallback === true);

  if (cursor < 0 && !allowAppendFallback) {
    Logger.log('[REPORT-HISTORY] skip status history: placeholder not found');
    return;
  }
  ...
  if (!historyGroups.length) {
    helpCompactReportParagraphV2_(insertParagraph('ไม่มีประวัติสถานะ'))
      .setFontFamily(font)
      .setFontSize(10);
    return;
  }
  ...
  if (cursor < 0) {
    ...
    helpCompactReportParagraphV2_(insertParagraph('ประวัติสถานะและเส้นทางเวลา (Timeline)'))
  ...
  const startHistoryTable = function(seqLabel) {
    helpCompactReportParagraphV2_(insertParagraph('เลขที่: ' + seqLabel), 4, 2)
      .setFontFamily(font)
      .setFontSize(10)
      .setBold(true);
    ...
    try {
      ...
      for (let c = 0; c < headerRow.getNumCells(); c++) {
        const cell = helpCompactReportCellV2_(headerRow.getCell(c), widths[c]);
        if (cell.getNumChildren() > 0) {
          helpCompactReportParagraphV2_(cell.getChild(0).asParagraph());
        }
      }
    } catch (e) {}
    currentHeight += 55;
  ...
  const appendTimelineImages = function(beforeIds, afterIds, sequenceNumber) {
    ...
    helpCompactReportParagraphV2_(insertParagraph('📷 ภาพประกอบงานซ่อม'))
      .setFontFamily(font)
      .setFontSize(9)
      .setBold(true);
    ...
    const renderImageCell = function(row, label, imageInfo) {
      if (!imageInfo) {
        helpCompactReportCellV2_(row.appendTableCell(''), 170);
        return;
      }
      const cell = helpCompactReportCellV2_(row.appendTableCell(''), 170);

      helpCompactReportParagraphV2_(cell.appendParagraph(label))
  ...
  historyGroups.forEach(function(group) {
    ...
    group.rows.forEach(function(row) {
      ...
      row.compactReportCells = true;
      appendRowToTable(currentTable, row, widths, font);
      try {
        const lastRow = currentTable.getRow(currentTable.getNumRows() - 1);
        for (let c = 0; c < lastRow.getNumCells(); c++) {
          const cell = helpCompactReportCellV2_(lastRow.getCell(c), widths[c]);
          for (let p = 0; p < cell.getNumChildren(); p++) {
            try {
              if (cell.getChild(p).asParagraph) helpCompactReportParagraphV2_(cell.getChild(p).asParagraph());
            } catch (e) {}
          }
        }
      } catch (e) {}
...
function appendReportImageSectionV2_(body, rows, font, options) {
  const sectionTarget = helpTakeReportPlaceholderTargetV2_(body, '{{SECTION_รูปภาพ}}');
  let sectionContainer = sectionTarget && sectionTarget.container ? sectionTarget.container : body;
  let sectionIndex = sectionTarget ? sectionTarget.index : -1;
  ...
  const insertParagraph = function(text) {
    const p = helpInsertReportParagraphV2_(sectionContainer, cursor, text);
    cursor++;
    return p;
  };
  const insertTable = function() {
    const table = helpInsertReportTableV2_(sectionContainer, cursor);
    cursor++;
    return table;
  };
  ...
  const appendImageOnlyCell = function(row, imageInfo, cellW, maxW, maxH) {
    const cell = helpCompactReportCellV2_(row.appendTableCell(''), cellW);

    const p = cell.appendParagraph('');
    helpCompactReportParagraphV2_(p).setAlignment(DocumentApp.HorizontalAlignment.CENTER);
    const img = p.appendInlineImage(imageInfo.blob);
    const origW = img.getWidth();
    const origH = img.getHeight();
    if (origW > 0 && origH > 0) {
  ...
  const appendImageGrid = function(title, images, label) {
    ...
    helpCompactReportParagraphV2_(insertParagraph(title))
      .setFontFamily(font)
      .setFontSize(9)
      .setBold(true);

    const columns = images.length >= 3 ? 3 : Math.min(2, images.length);
    const cellW = columns === 3 ? 120 : (columns === 2 ? 170 : 180);
    const maxW = columns === 3 ? 105 : (columns === 2 ? 130 : 140);
    const maxH = columns === 3 ? 110 : 120;
    const imageTable = insertTable();
  ...
  if (helpInsertReportPageBreakAtCursorV2_(sectionContainer, cursor)) cursor++;

  helpCompactReportParagraphV2_(insertParagraph('รูปภาพประกอบ'))
    .setFontFamily(font)
    .setFontSize(11)
    .setBold(true);
  ...
  resolvedGroups.forEach(function(group) {
    ...
    helpCompactReportParagraphV2_(insertParagraph('เลขที่: ' + group.seqLabel))
      .setFontFamily(font)
      .setFontSize(10)
      .setBold(true);
...
function cleanupTrailingBlankContent_(body) {
  if (!body) return;
  try {
    while (body.getNumChildren() > 0) {
      const lastIndex = body.getNumChildren() - 1;
      const child = body.getChild(lastIndex);
      const type = child.getType && child.getType();

      if (type === DocumentApp.ElementType.PARAGRAPH) {
        const text = child.asParagraph().getText().replace(/\s+/g, ' ').trim();
        if (!text) {
          body.removeChild(child);
          continue;
        }
    ...
    Logger.log('[WARN] cleanupTrailingBlankContent_ failed: ' + e.message);
  }
}
...
// ----------------------------------------------------------------
// SECTION: 📡 API ROUTER FOR EXTERNAL FRONTEND (doPost)
// ----------------------------------------------------------------
function doPost(e) {
  try {
    if (!e || !e.postData || !e.postData.contents) {
      throw new Error("ไม่พบข้อมูลส่งเข้าระบบ (No post data)");
    }
    ...
    // รายการฟังก์ชันที่อนุญาตให้เรียกใช้ภายนอกเพื่อความปลอดภัย (Whitelist)
    const allowedActions = [
      ...
      'updateRepairStatus',
      'deleteRepair',
      'getTicketRepairCosts',
      'getSparePartsList',
      'getRecentStockTransactions',
      'getRepairJobOptionsForStock',
      'getAvailableStockReportMonths',
      'reviewWithdrawalRequest',
      'getPendingWithdrawalRequestCountForDashboard',
      'getDashboardData',
      'apiHealthCheck',
      'getUrl',
      'getTechnicians',
      ...
      'selfTest',
      'getWithdrawalRequestsForRepair',
      'getAvailableSparePartsForWithdrawal',
      'createWithdrawalRequestForRepair',
      'getWithdrawalRequestsForCurrentUser',
      'generateMonthlyStockReport',
      'deleteSparePartRow',
      'updateSparePartRow',
      'processStockTransaction',
      'searchMyRepairTickets',
      'enqueuePdf',
      'pdfWorker',
      'getLogoPublicUrl'
    ];
    ...
    if (allowedActions.indexOf(action) === -1) {
      throw new Error("ไม่อนุญาตให้เรียกใช้ฟังก์ชัน: " + action);
    }
    
    // ค้นหาฟังก์ชันใน Global Scope ของ Apps Script
    const fn = globalThis[action] || this[action];
    if (typeof fn !== 'function') {
      throw new Error("ไม่พบฟังก์ชันบนเซิร์ฟเวอร์: " + action);
    }
    
    // รันฟังก์ชันและส่งกลับผลลัพธ์
...
/**
 * ฟังก์ชันสำหรับตรวจสุขภาพการเชื่อมต่อ API ระหว่าง GitHub Pages และ Apps Script
 */
